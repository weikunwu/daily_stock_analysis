...
> 说明：`market_review_payload` 中的 `breadth` 仅在行情宽度数据真实可用时下发；美股宽度来自 TickFlow US_Equity 标的池聚合（需「标的池查询」权限，免费档未开通或请求失败时不下发），港股宽度暂无数据源，均不下发该字段。前端显示层需按“字段缺失”降级为“暂无数据”而不是展示 0。
